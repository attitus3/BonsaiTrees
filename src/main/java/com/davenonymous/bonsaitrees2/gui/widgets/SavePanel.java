...
public class SavePanel extends WidgetPanel {
    ...
    public SavePanel(int width, int height, MultiblockBlockModel model) {
        ...
        WidgetButton saveToDiskButton = new WidgetButton(I18n.get("bonsaitrees.gui.tree_creator.button.save_to_disk"));
        ...
        WidgetTextBox overwriteFilesLabel = new WidgetTextBox(I18n.get("bonsaitrees.gui.tree_creator.checkbox.overwrite_files"), 0xC0000000);
        overwriteFilesLabel.setPosition(12, 25);
        this.add(overwriteFilesLabel);

        WidgetTextBox openloaderHint = new WidgetTextBox(I18n.get("bonsaitrees.gui.tree_creator.hint.openloader"), 0x90000000);
        ...
        WidgetButton openOpenLoaderPage = new WidgetButton(I18n.get("bonsaitrees.gui.tree_creator.button.go_to_openloader"));
        ...
        openOpenLoaderPage.addListener(MouseClickEvent.class, (event, widget) -> {
            Util.getPlatform ().openUri(openLoaderUrl);
            return WidgetEventResult.HANDLED;
        });
    ...
    private void saveToDisk() {
        ...
        ResourceLocation treeId = ResourceLocation.tryParse(treeIdString);
        ...
        if(treeIdString.length() == 0) {
            statusBox.setText(I18n.get("bonsaitrees.gui.tree_creator.warning.missing_tree_id"));
            return;
        }

        if(model.blocks.size() >= 4095) {
            statusBox.setText(I18n.get("bonsaitrees.gui.tree_creator.warning.too_many_blocks"));
            return;
        }

        if(model.refMap.size() > 26) {
            statusBox.setText(I18n.get("bonsaitrees.gui.tree_creator.warning.too_many_types"));
            return;
        }

        File assetPath = new File(Minecraft.getInstance().gameDirectory, "openloader/resources/custombonsais/assets/" + BonsaiTrees2.MODID + "/models/tree/" + treeId.getNamespace());
        assetPath.mkdirs();

        File recipePath = new File(Minecraft.getInstance().gameDirectory, "openloader/data/custombonsais/data/" + BonsaiTrees2.MODID + "/recipes/sapling/" + treeId.getNamespace());
        recipePath.mkdirs();

        File datapackMetaFile = new File(Minecraft.getInstance().gameDirectory, "openloader/data/custombonsais/pack.mcmeta");
        File resourcepackMetaFile = new File(Minecraft.getInstance().gameDirectory, "openloader/resources/custombonsais/pack.mcmeta");
        ...
        if((recipeFilePath.exists() || modelPath.exists()) && !overwriteFiles.getValue()) {
            statusBox.setText(I18n.get("bonsaitrees.gui.tree_creator.warning.files_already_exists"));
            return;
        }

        model.id = new ResourceLocation(BonsaiTrees2.MODID, "sapling/" + treeIdPath);

        SaplingInfo saplingInfo = new SaplingInfo(model.id, Ingredient.of(sapling), baseTicks);
        ...
        statusBox.setText(I18n.get("bonsaitrees.gui.tree_creator.label.saved_files"));
    }
